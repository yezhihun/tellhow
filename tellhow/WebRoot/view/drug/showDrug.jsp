...
<html>
  ...
  $(function(){
		var store = $.souiStore.create({
	        url: '${pageContext.request.contextPath}/drug/ajaxShowList.action',
	        root: 'list',
		...
		var grid = $("#grid-panel").souiGrid({
	        ...
	        store: store,
	        columns: [
		        {label: "名称", width: 150, key: 'name'},
		        {label: "描述", width: 150, key: 'info'},
		        {label: "操作", width: 150, renderer: function(data){
		        	var updateUrl = "'${pageContext.request.contextPath}/drug/updateDrug.action?drug.id="+data.id+"'";
		        	var deleteUrl = "'${pageContext.request.contextPath}/drug/ajaxDeleteDrug.action?drug.id="+data.id+"'";
	   ...
	   $("#dialog-form").dialog({
			...
			buttons: {
				"OK": function() {
			  		var _url = "${pageContext.request.contextPath}/drug/ajaxUpdateDrug.action";
  ...
  <body>
  	<div id="query-panel" style="margin:25px 25px 25px 25px">
  		<label>名称：</label>
  		<input type="text" id="name" name="drug.name">
		...
		<div id="query">
	  	<table>
	  		<tr>
	  			...
	  			<td> <input type="text" id="newname" name="drug.name" value="${drug.name}"> <input id="id" name="drug.id" value="${drug.id}" type="text" style="display: none;" > <td> 
	  		</tr>
	  		<tr>
	  			<td><span>描述</span><td>
	  			<td> <input type="text" if="info" name="drug.info" value="${drug.info}"> <td> 
	  		</tr>
	  	</table>
  	</div>
	</div>
