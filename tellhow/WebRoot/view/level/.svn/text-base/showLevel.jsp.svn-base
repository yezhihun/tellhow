...
<html>
  ...
  $(function(){
		var store = $.souiStore.create({
	        url: '${pageContext.request.contextPath}/level/ajaxShowList.action',
	        root: 'list',
		...
		var grid = $("#grid-panel").souiGrid({
	        ...
	        store: store,
	        columns: [
		        {label: "级别", width: 150, key: 'name'},
		        {label: "描述", width: 150, key: 'info'},
		        {label: "操作", width: 150, renderer: function(data){
		        	var updateUrl = "'${pageContext.request.contextPath}/level/updateLevel.action?level.id="+data.id+"'";
		        	var deleteUrl = "'${pageContext.request.contextPath}/level/ajaxDeleteLevel.action?level.id="+data.id+"'";
  ...
  <body>
  	<div id="query-panel" style="margin:25px 25px 25px 25px">
  		<label>级别：</label>
  		<input type="text" id="name" name="level.name">
	...
	<div id="dialog-form" title="编辑">
		<div id="query">
	  	<table>
	  		<tr>
	  			<td><span>级别</span><td>
	  			<td> <input type="text" id="newname" name="level.name" value="${level.name}"> <input id="id" name="level.id" value="${level.id}" type="text" style="display: none;" > <td> 
	  		</tr>
	  		<tr>
	  			<td><span>描述</span><td>
	  			<td> <input type="text" id="info" name="level.info" value="${level.info}"> <td> 
	  		</tr>
	  	</table>
  	</div>
	</div>
