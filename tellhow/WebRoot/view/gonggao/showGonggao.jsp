...
<!--	<script type="text/javascript" src="${pageContext.request.contextPath}/scripts/js/jquery-1.4.2.min.js"></script>-->
  ...
  $(function(){
		var store = $.souiStore.create({
	        url: '${pageContext.request.contextPath}/gonggao/ajaxShowList.action',
	        root: 'list',
		...
		var grid = $("#grid-panel").souiGrid({
	        ...
	        columns: [
		        ...
		        {label: "操作", width: 150, renderer: function(data){
		        	var updateUrl = "'${pageContext.request.contextPath}/gonggao/updateGongGao.action?gongGao.id="+data.id+"'";
		        	var deleteUrl = "'${pageContext.request.contextPath}/gonggao/ajaxDeleteGongGao.action?gongGao.id="+data.id+"'";
	    ...
	    $("#dialog-form").dialog({
			...
			buttons: {
				"OK": function() {
			  		var _url = "${pageContext.request.contextPath}/gonggao/ajaxUpdateGongGao.action";
  ...
  <body>
  	<div id="query-panel" style="margin:25px 25px 25px 25px">
  		<label>级别：</label>
  		<input type="text" id="title" name="gongGao.title">
	...
	<form>
	<fieldset id="query">
		<label>标题</label><input type="text" id="id" name="gongGao.id" style="display: none;">
		<input type="text" name="gongGao.title" id="newtitle"  /><br>
		<label>正文</label>
		<textarea rows="5" cols="30" name="gongGao.info" id="info"></textarea>
	</fieldset>
	</form>
	</div>
</body>
